...
Claude Code Skills 和 MCP 服务器的可视化管理工具 —— 在浏览器中启用、禁用、删除、批量操作 skills 和 MCP servers，支持用户/项目/插件多个作用域。
...
```bash
# 查看当前目录对应项目的 skills 和 MCP
claude-manager

# 查看指定项目的 skills 和 MCP
claude-manager /path/to/my-project

# 指定端口
PORT=8080 claude-manager
```
...
切换到 **MCP** 标签页，可管理 MCP 服务器配置：

- **USER MCP** —— `~/.claude.json` 中的 `mcpServers`
- **PROJECT MCP** —— 当前项目 `.claude/mcp.json` 中的 `mcpServers`

禁用时将服务器配置移入 `_disabledMcpServers` 键（Claude Code 忽略此键），启用时移回 `mcpServers`。
...
顶部搜索框实时按名称过滤，各区块同步筛选。

### 删除

点击行尾的垃圾桶图标，弹出确认对话框后执行删除。**删除操作不可撤销。**

Plugin Skills 不支持删除（需通过 Claude Code 插件管理器卸载插件），删除按钮隐藏。

### 批量操作

勾选每行左侧的复选框可选中多个条目；点击区块标题处的复选框可全选/全不选当前区块（支持半选状态）。

选中 1 个及以上时，页面底部弹出操作栏：

| 按钮 | 说明 | 灰掉条件 |
|------|------|----------|
| **Enable** | 批量启用所有选中的已禁用条目 | 选中的全部已启用 |
| **Disable** | 批量禁用所有选中的已启用条目 | 选中的全部已禁用 |
| **Delete** | 弹出确认框，列出所有选中条目后批量删除 | 永远可点 |
| **✕** | 清空选中 | — |
...
插件 skills 来自 `~/.claude/plugins/installed_plugins.json` 中记录的已安装插件。每个插件可能在以下位置同时存有 skill 文件，Claude Manager 全部纳入管理：
...
**重要**：Claude Code 可能从上述任意位置加载同一个 plugin skill。若只禁用 `cache/` 中的副本而 `marketplaces/` 副本仍活跃，Claude Code 依然会加载该 skill。Claude Manager 的 enable/disable 操作会自动同步所有副本。
...
- Claude Manager 将每个 sub-skill（`pua-en`、`pua-ja`、`shot` 等）单独显示
- 因此 Claude Manager 显示的数量通常多于 Claude Code 的 `/skills` 计数
...
| 查看其他项目的 Project Skills | 传入项目路径参数：`claude-manager /path/to/project` |
...
测试覆盖 Express API 的全部端点：skill 扫描、启用/禁用/删除、description 字段提取（含块标量和禁用 skill）、MCP 服务器扫描与操作。
...
| 命令 | 说明 |
|------|------|
| `claude-manager` | 全局安装后，查看当前目录的 skills 和 MCP |
| `claude-manager <path>` | 查看指定项目的 skills 和 MCP |
